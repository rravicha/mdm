...
locals {
  name = "adl-enriched-gbl-cf-customer-mdm-sfdc-hcp-api" 
...
EOF
  ...
  helpers = {
    ...
    {
      state = "enrichedmdm-sfdc"
      type = "classic"
      next = "proceed_further"
      flow = [
        {
          name            = "trust_to_enrich_sfdc_hcp_api" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/api_sfdc_hcp.py"
