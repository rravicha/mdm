...
locals {
  name             = "adl-trusted-gbl-cf-customer-mdm-dnb"
  ...
  workflow_sm = [
    ...
    {
      state = "turstedmdm-dnb"
      type = "classic"
      next = "proceed_further"
      flow = [
        {
          name            = "DNB_trusted" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/tst_dnb_hco.py"
