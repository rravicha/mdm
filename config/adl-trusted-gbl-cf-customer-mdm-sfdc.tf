variable "rw_buckets" {
  type = list(string)
  default = [
    "adl-customer-mdm-etl/adl-raw-gbl-cf-customer-mdm","adl-customer-mdm-etl/adl-trusted-gbl-cf-customer-mdm"
  ]
}
...
locals {
  name             = "adl-trusted-gbl-cf-customer-mdm-sfdc"
  description      = "adl-trusted-gbl-cf-customer-mdm"
  ...
  workflow_sm = [
    ...
    {
      state = "turstedmdm-sfdc"
      type = "classic"
      next = "proceed_further"
      flow = [
        {
          name            = "SFDC_trusted" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/tst_sfdc_hco_hcp.py"
