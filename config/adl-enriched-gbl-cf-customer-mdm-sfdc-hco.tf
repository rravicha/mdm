...
locals {
  name = "adl-enriched-gbl-cf-customer-mdm-sfdc-hco" 
...
EOF
  ...
  helpers = {
...

    {
      ...
      flow = [
        {
          name            = "trust_to_enrich_sap_hco" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/enr_sfdc_hco.py"
