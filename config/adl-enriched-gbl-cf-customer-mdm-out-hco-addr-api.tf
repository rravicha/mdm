...
locals {
  name = "adl-enriched-gbl-cf-customer-mdm-out-hco-addr-api" 
...
EOF
  ...
  helpers = {
...

    {
      ...
      flow = [
        {
          name            = "trust_to_enrich_out_hco_addr_api" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/out_api_hco_address.py"
