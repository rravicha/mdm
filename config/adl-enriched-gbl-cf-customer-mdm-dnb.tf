...
locals {
  name = "adl-enriched-gbl-cf-customer-mdm-dnb" 
...
EOF
  ...
  helpers = {
    ...
    {
      state = "enrichedmdm-dnb"
      type = "classic"
      next = "proceed_further"
      flow = [
        {
          name            = "trust_to_enrich" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/enr_dnb_hco.py"
