...
locals {
  name = "adl-enriched-gbl-cf-customer-mdm-out-hcp-addr-api" 
...
EOF
  ...
  helpers = {
...

    {
      ...
      flow = [
        {
          name            = "trust_to_enrich_out_hcp_addr_api" // Job name
          type            = "glueetl"      // [ glueetl || pythonshell || python3.7 ]
          glue_version    = "4.0"
          script_location = "adl-customer-mdm-etl/releases/${var.artifacts_version}/outbound_api_hcp_address.py"
